...
const MAX_RETRIES = 5;
const RETRY_DELAY_MS = 2000; // 2 secondes entre chaque tentative
...
async function connectWithRetry(retryCount = 0) {
    ...
    try {
        // Vérifier l'accès au répertoire parent
        ...
        try {
            ...
            throw new Error(`Impossible d'accéder au répertoire réseau: ${accessError.message}`);
        }
        ...
        connectionAttempts++;
        console.error(`❌ Tentative ${connectionAttempts}/${MAX_RETRIES} - Erreur de connexion:`, error.message);
        ...
        if (retryCount < MAX_RETRIES) {
            ...
            console.error(`❌ ÉCHEC CONNEXION BASE DE DONNÉES après ${MAX_RETRIES} tentatives`);
            console.error(`   Chemin: ${dbPath}`);
            console.error(`   Solution: Vérifiez l'accès réseau à \\\\192.168.1.230`);
            throw error;
        }
    }
}
...
module.exports = { connect, connectWithRetry, close, run, get, all, prepare, transaction, exec, getConnection };